...
from efsc.data.collator import PreferenceCollator
from efsc.data.dataset import PreferenceDataset
from efsc.losses import pairwise_dpo_action_loss
...
def main() -> None:
    ...
    ds = PreferenceDataset(config["train_path"])
    collator = PreferenceCollator(model.tokenizer, max_length=int(config.get("max_length", 512)))
    ...
    total_steps = int(config.get("epochs", 2)) * max(1, math.ceil(len(loader) / grad_accum))
    optimizer, scheduler = build_optimizer_scheduler(model, config, total_steps)
    history = []; beta = float(config.get("beta", 1.0)); best_loss = float("inf")
    ...
    for epoch in range(int(config.get("epochs", 2))):
        ...
        model.train()
        optimizer.zero_grad()
        for step, batch in enumerate(tqdm(loader, desc=f"stage3 epoch {epoch+1}"), start=1):
            batch = move_batch_to_device(batch, device)
            with torch.cuda.amp.autocast(enabled=scaler.is_enabled()):
                outputs = model(input_ids=batch["input_ids"], attention_mask=batch["attention_mask"])
                loss = pairwise_dpo_action_loss(outputs.action_logits, batch["preferred_action_ids"], batch["dispreferred_action_ids"], beta=beta) / grad_accum
        ...
        save_json({"history": history}, Path(output_dir) / "stage3_history.json")
        write_history_csv(history, Path(output_dir) / "stage3_history.csv")
        torch.save(model.state_dict(), Path(output_dir) / "stage3_last.pt")
        if history[-1]["loss"] < best_loss:
            best_loss = history[-1]["loss"]; torch.save(model.state_dict(), Path(output_dir) / "stage3_best.pt")

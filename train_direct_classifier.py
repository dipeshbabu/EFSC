...
from src.models.efsc_model import DirectActionClassifier
from src.models.model_factory import build_direct_model
from src.train.collators import efsc_collate_fn
from src.train.utils import save_json, set_seed, to_device


@torch.no_grad()
def evaluate(model: DirectActionClassifier, loader: DataLoader, device: torch.device) -> Dict[str, float]:
    model.eval()
    total_loss = 0.0
    n = 0
    for batch in loader:
        ...
        loss = F.cross_entropy(out["action_logits"], batch["action_label"])
...
def main() -> None:
    ...
    for epoch in range(args.epochs):
        ...
        if row["val_loss"] < best_val:
            ...
            torch.save(model.state_dict(), out_dir / "best_direct_classifier.pt")

    torch.save(model.state_dict(), out_dir / "last_direct_classifier.pt")
    save_json({"history": history}, out_dir / "train_log_direct_classifier.json")

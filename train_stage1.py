...
from src.train.losses import stage1_supervised_loss
from src.train.utils import save_json, set_seed, to_device


@torch.no_grad()
def evaluate(model: EFSCModel, loader: DataLoader, device: torch.device) -> Dict[str, float]:
    model.eval()
    total_loss = 0.0
    n = 0
    for batch in loader:
...
def main() -> None:
    ...
    for epoch in range(args.epochs):
        ...
        if row["val_loss"] < best_val:
            ...
            torch.save(model.state_dict(), out_dir / "best_stage1.pt")

    torch.save(model.state_dict(), out_dir / "last_stage1.pt")
    save_json({"history": history}, out_dir / "train_log_stage1.json")

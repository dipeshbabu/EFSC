...
def main() -> None:
    ...
    retention_before = out_root / "retention" / f"{args.run_id}__{args.quant_mode}__before__{args.dataset_name}.json"
    retention_after = out_root / "retention" / f"{args.run_id}__{args.quant_mode}__after__{args.dataset_name}.json"

    train_cmd = [
        "python", "train_qefsc_fc.py",
    ...
    run_if_missing(fp_pred, [
        "python", "run_inference_qefsc_fc.py",
        "--model_name", args.model_name,
        "--variant", args.variant,
    ...
    run_if_missing(q_pred, [
        "python", "run_quantized_inference_qefsc_fc.py",
        "--model_name", args.model_name,
    ...
    run_if_missing(retention_before, [
        ...
        "--output", str(retention_before),
    ])

    run_if_missing(recal_ckpt, [
        "python", "recalibrate_quantized_controller.py",
        "--model_name", args.model_name,
        "--quant_mode", args.quant_mode,
        "--controller_checkpoint", str(best_ckpt),
        "--train_path", args.train_path,
    ...
    run_if_missing(retention_after, [
        "python", "scripts/evaluate_quant_retention.py",
        "--fp_metrics", str(fp_metrics),
        "--quant_metrics", str(qr_metrics),
        "--fp_preds", str(fp_pred),
        "--quant_preds", str(qr_pred),
        "--output", str(retention_after),
    ])
    ...
    print(f"Completed quantization experiment {args.run_id} on {args.dataset_name}")
